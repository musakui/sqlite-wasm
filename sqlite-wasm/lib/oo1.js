import { capi, structs, sqliteError, sqlite3_js_rc_str, cstrToJs, C_API } from './base.js'
...
import { sqlite3_js_db_vfs } from './wasm.js'
...
const affirmParamIndex = (stmt, key) => {
	const n = 'number' === typeof key ? key : capi_m.sqlite3_bind_parameter_index(stmt.pointer, key)
...
const bindOne = function f(stmt, ndx, bindType, val) {
	...
	switch (null === val || undefined === val ? BindTypes.null : bindType) {
		case BindTypes.null:
			rc = capi_m.sqlite3_bind_null(stmt.pointer, ndx)
			break
		case BindTypes.string:
		...
		case BindTypes.number: {
			let m
			if (util.isInt32(val)) {
				m = capi_m.sqlite3_bind_int
			} else if ('bigint' === typeof val) {
				if (!util.bigIntFits64(val)) {
					f._tooBigInt(val)
				} else {
					m = capi_m.sqlite3_bind_int64
				}
			} else {
				val = Number(val)
				m = Number.isInteger(val) ? capi_m.sqlite3_bind_int64 : capi_m.sqlite3_bind_double
			}
			rc = m(stmt.pointer, ndx, val)
			break
		}
		case BindTypes.boolean:
			rc = capi_m.sqlite3_bind_int(stmt.pointer, ndx, val ? 1 : 0)
			break
		case BindTypes.blob: {
			...
			const pBlob = heap.alloc(val.byteLength || 1)
...
export const installOO1 = (sqlite3) => {
	const __dbTraceToConsole = heap.installFunction(
		'i(ippp)',
		function (t, c, p, x) {
			if (C_API.SQLITE_TRACE_STMT === t) {
				logger.info('SQL TRACE #' + ++this.counter + ' via sqlite3@' + c + ':', cstrToJs(x))
			}
		}.bind({ counter: 0 })
	)
	...
	const checkSqlite3Rc = function (dbPtr, sqliteResultCode) {
		if (sqliteResultCode) {
			...
			sqliteError(
				...
				'sqlite3 result code',
				sqliteResultCode + ':',
				dbPtr ? capi_m.sqlite3_errmsg(dbPtr) : capi_m.sqlite3_errstr(sqliteResultCode)
			)
		}
		return arguments[0]
	}
	...
	const dbCtorHelper = function ctor(...args) {
		...
		try {
			...
			let rc = capi_m.sqlite3_open_v2(fn, pPtr, oflags, vfsName || 0)
			pDb = heap.peekPtr(pPtr)
			checkSqlite3Rc(pDb, rc)
			capi_m.sqlite3_extended_result_codes(pDb, 1)
	...
	class DB {
		...
		changes(total = false, sixtyFour = false) {
			...
			if (total) {
				return sixtyFour ? capi.sqlite3_total_changes64(p) : capi_m.sqlite3_total_changes(p)
			} else {
				return sixtyFour ? capi.sqlite3_changes64(p) : capi_m.sqlite3_changes(p)
			}
		}
		dbFilename(dbName = 'main') {
			return capi_m.sqlite3_db_filename(affirmDbOpen(this).pointer, dbName)
		}
		dbName(dbNumber = 0) {
			return capi_m.sqlite3_db_name(affirmDbOpen(this).pointer, dbNumber)
		}
		dbVfsName(dbName = 0) {
			let rc
		...
		exec() {
			...
			try {
				...
				let sqlByteLen = isTA ? arg.sql.byteLength : heap.jstrlen(arg.sql)
				...
				else heap.jstrcpy(arg.sql, heap.heap8(), pSql, sqlByteLen, false)
				...
				while (pSql && heap.peek(pSql, 'i8')) {
					...
					if (!pStmt) continue
					if (saveSql) saveSql.push(capi_m.sqlite3_sql(pStmt).trim())
	...
	class Stmt {
		constructor() {
			...
			this.parameterCount = capi_m.sqlite3_bind_parameter_count(this.pointer)
		}
		...
		get columnCount() {
			return capi_m.sqlite3_column_count(this.pointer)
		}

		finalize() {
			if (this.pointer) {
				affirmNotLockedByExec(this, 'finalize()')
				const rc = capi_m.sqlite3_finalize(this.pointer)
		...
		clearBindings() {
			...
			capi_m.sqlite3_clear_bindings(this.pointer)
			this._mayGet = false
			return this
		}
		reset(alsoClearBinds) {
			...
			const rc = capi_m.sqlite3_reset(affirmStmtOpen(this).pointer)
			this._mayGet = false
		...
		step() {
			...
			const rc = capi_m.sqlite3_step(affirmStmtOpen(this).pointer)
			switch (rc) {
				...
				default:
					this._mayGet = false
					logger.warn('sqlite3_step() rc=', rc, sqlite3_js_rc_str(rc), 'SQL =', capi_m.sqlite3_sql(this.pointer))
		...
		get(ndx, asType) {
			...
			if (Array.isArray(ndx)) {
				...
				const n = this.columnCount
				while (i < n) {
					ndx[capi_m.sqlite3_column_name(this.pointer, i)] = this.get(i++)
				}
				return ndx
			}
			affirmColIndex(this, ndx)
			switch (undefined === asType ? capi_m.sqlite3_column_type(this.pointer, ndx) : asType) {
				case C_API.SQLITE_NULL:
					return null
				case C_API.SQLITE_INTEGER: {
					const rc = capi_m.sqlite3_column_int64(this.pointer, ndx)
				...
				case C_API.SQLITE_FLOAT:
					return capi_m.sqlite3_column_double(this.pointer, ndx)
				case C_API.SQLITE_TEXT:
					return capi_m.sqlite3_column_text(this.pointer, ndx)
				case C_API.SQLITE_BLOB: {
					const n = capi_m.sqlite3_column_bytes(this.pointer, ndx),
						ptr = capi_m.sqlite3_column_blob(this.pointer, ndx),
						rc = new Uint8Array(n)

					if (n) rc.set(heap.heap8u().slice(ptr, ptr + n), 0)
		...
		getColumnName(ndx) {
			return capi_m.sqlite3_column_name(affirmColIndex(affirmStmtOpen(this), ndx).pointer, ndx)
		}
		getColumnNames(tgt = []) {
			affirmColIndex(affirmStmtOpen(this), 0)
			const n = this.columnCount
			for (let i = 0; i < n; ++i) {
				tgt.push(capi_m.sqlite3_column_name(this.pointer, i))
			}
			return tgt
		}
		getParamIndex(name) {
			return affirmStmtOpen(this).parameterCount ? capi_m.sqlite3_bind_parameter_index(this.pointer, name) : undefined
		}
	}
