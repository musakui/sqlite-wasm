...
export const installOpfsVfs = (sqlite3, options) => {
	if (!globalThis.SharedArrayBuffer || !globalThis.Atomics) {
		throw new Error('Cannot install OPFS: See https://sqlite.org/wasm/doc/trunk/persistence.md#coop-coep')
	}
	if (typeof WorkerGlobalScope === 'undefined') throw new Error('The OPFS sqlite3_vfs requires Atomics.wait()')
	if (!thisThreadHasOPFS()) throw new Error('Missing required OPFS APIs.')
	...
	return new Promise((promiseResolve_, promiseReject_) => {
		const loggers = [sqlite3.config.error, sqlite3.config.warn, sqlite3.config.log]
		const logImpl = (level, ...args) => {
			if (verbose > level) loggers[level]('OPFS syncer:', ...args)
		}
		...
		const opfsVfs = new capi.sqlite3_vfs().addOnDispose(() => opfsIoMethods.dispose())
		...
		const W = new Worker(new URL('sqlite3-opfs-async-proxy.js', import.meta.url), { type: 'module' })
				...
				if (argc) {
					...
					for (i = 0; i < argc; ++i) {
						...
						if (t.getter) {
							...
							offset += 4
							v = DECODER.decode(viewU8.slice(offset, offset + n))
							offset += n
						}
						rc.push(v)
					}
		...
		if (!opfsVfs.$xSleep) {
			vfsSyncWrappers.xSleep = (pVfs, ms) => {
		...
		opfsUtil.registerVfs = (def = false) => wasm.exports.sqlite3_vfs_register(opfsVfs.pointer, def ? 1 : 0)
		...
		async function importDbChunked(filename, callback) {
		...
		if (sqlite3.oo1) {
			class OpfsDb extends sqlite3.oo1.DB {
				constructor(...args) {
					super()
			...
			sqlite3.oo1.DB.dbCtorHelper.setVfsPostOpenSql(opfsVfs.pointer, function (oo1Db, sqlite3) {
				...
				sqlite3.capi.sqlite3_exec(oo1Db, ['pragma journal_mode=DELETE;', 'pragma cache_size=-16384;'], 0, 0, 0)
			})
		}
