...
import { capi, wasm, getMemory, sqliteError, allocError } from './init.js'
import {
	xArg,
	xResult,
	functionTable,
...
import { sqlite3_wasm_db_error } from './capi_with_export.js'
import { ptrSizeof, ptrIR, isLittleEndian } from './constants.js'
import * as logger from './logger.js'

const toss = utils.toss
...
const Jaccwabyt = function StructBinderFactory(config) {
	...
	const __memberToJsString = function f(obj, memberName) {
		...
		return addr === pos ? '' : utils.typedArrayToString(mem, addr, pos)
	}
...
export const installWhWasm = (sqlite3, exports) => {
	...
	wasm.allocFromTypedArray = function (src) {
		...
		if (!utils.isBindableTypedArray(src)) sqliteError('Value is not of a supported TypedArray type.')
...

	{
		const flexibleString = function (v) {
			if (utils.isSQLableTypedArray(v)) return utils.typedArrayToString(v instanceof ArrayBuffer ? new Uint8Array(v) : v)
			if (Array.isArray(v)) return v.join('')
			return utils.isPtr(v) ? wasm.cstrToJs(v) : v
		}
	...
	const __argStr = (str) => (utils.isPtr(str) ? wasm.cstrToJs(str) : str)
...

	{
		const __flexiString = (v, n) => {
			if ('string' === typeof v) {
				n = -1
			} else if (utils.isSQLableTypedArray(v)) {
				n = v.byteLength
				v = utils.typedArrayToString(v instanceof ArrayBuffer ? new Uint8Array(v) : v)
...

	{
		...
		capi.sqlite3_bind_text = function f(pStmt, iCol, text, nText, xDestroy) {
			...
			} else if (utils.isPtr(text) || null === text) {
			...
			let p, n
			try {
				if (utils.isSQLableTypedArray(text)) {
		...
		capi.sqlite3_bind_blob = function f(pStmt, iCol, pMem, nMem, xDestroy) {
			...
			} else if (utils.isPtr(pMem) || null === pMem) {
			...
			let p, n
			try {
				if (utils.isBindableTypedArray(pMem)) {
...

	{
		...
		capi.sqlite3_auto_extension = function (fPtr) {
			...
			} else if (1 !== arguments.length || !utils.isPtr(fPtr)) {
		...
		capi.sqlite3_cancel_auto_extension = function (fPtr) {
			if (!fPtr || 1 !== arguments.length || !utils.isPtr(fPtr)) return 0
...
function WhWasmUtilInstaller(target) {
	...
	target.cstrlen = function (ptr) {
		if (!ptr || !utils.isPtr(ptr)) return null
	...
	target.cstrToJs = function (ptr) {
		...
		return n ? utils.typedArrayToString(heapWrappers().HEAP8U, ptr, ptr + n) : null === n ? n : ''
	}
	...
	xArg.FuncPtrAdapter = class FuncPtrAdapter extends AbstractArgAdapter {
		...
		convertArg(v, argv, argIndex) {
			...
			if (v instanceof Function) {
				...
				return fp
			} else if (utils.isPtr(v) || null === v || undefined === v) {
