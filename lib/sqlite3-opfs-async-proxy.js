	...
	if (1) {
		return e instanceof GetSyncHandleError &&
			(e.cause.name === 'NoModificationAllowedError' ||
				/* Inconsistent exception.name from Chrome/ium with the same exception.message text: */
...
const vfsAsyncImpls = {
    ...
    */
	...
	xRead: async function (fid /*sqlite3_file pointer*/, n, offset64) {
		...
		try {
			wTimeStart('xRead')
			nRead = (await getSyncHandle(fh, 'xRead')).read(fh.sabView.subarray(0, n), {
				at: Number(offset64),
			})
			wTimeEnd()
			if (nRead < n) {
	...
	state.s11n.serialize = function (...args) {
		...
		if (args.length) {
			...
			for (; i < args.length; ++i, ++offset) {
				/* Write the TypeIds.id value into the next args.length bytes. */
				typeIds.push(getTypeId(args[i]))
				viewU8[offset] = typeIds[i].id
			}
			for (i = 0; i < args.length; ++i) {
				/* Deserialize the following bytes based on their corresponding TypeIds.id from the header. */
				const t = typeIds[i]
				if (t.setter) {
	...
	while (!flagAsyncShutdown) {
		try {
			if ('not-equal' !== Atomics.wait(state.sabOPView, state.opIds.whichOp, 0, state.asyncIdleWaitTime)) {
				/* Maintenance note: we compare against 'not-equal' because https://github.com/tomayac/sqlite-wasm/issues/12 */
			...
			// clear s11n to keep the caller from confusing this with an exception string written by the upcoming operation
...
self.onmessage = ({ data }) => {
	switch (data.type) {
		case 'opfs-async-init': {
			...
			for (const k of Object.keys(vfsAsyncImpls)) {
