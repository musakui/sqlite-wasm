import { capi, wasm, sqliteError } from './init.js'
...
export const installOO1 = (sqlite3) => {
	...
	class Stmt {
		constructor() {
			if (BindTypes !== arguments[2]) {
				sqliteError(capi.SQLITE_MISUSE, 'Do not call the Stmt constructor directly. Use DB.prepare().')
			}
			this.db = arguments[0]
		...
		step() {
			...
			switch (rc) {
				...
				default:
					this._mayGet = false
					logger.warn('sqlite3_step() rc=', rc, capi.sqlite3_js_rc_str(rc), 'SQL =', capi.sqlite3_sql(this.pointer))
	...
	const affirmDbOpen = function (db) {
		if (!db.pointer) sqliteError('DB has been closed.')
		return db
	}
				...
				default:
					if (util.isInt32(opt.rowMode)) {
						...
						out.cbArg = (stmt) => {
							...
							return undefined === rc ? sqliteError(capi.SQLITE_NOTFOUND, 'exec(): unknown result column:', $colName) : rc
						}
						break
					}
	...
	DB.prototype = {
		...
		exec: function () {
			...
			try {
				...
				const ppStmt = wasm.scopedAlloc(2 * ptrSizeof + (sqlByteLen + 1))
				const pzTail = ppStmt + ptrSizeof
				let pSql = pzTail + ptrSizeof
				const pSqlEnd = pSql + sqlByteLen
				if (isTA) wasm.heap8().set(arg.sql, pSql)
				else util.jstrcpy(arg.sql, wasm.heap8(), pSql, sqlByteLen, false)
		...
		transaction: function (callback) {
			...
			if (arguments.length > 1) {
				if (/[^a-zA-Z]/.test(arguments[0])) {
					sqliteError(capi.SQLITE_MISUSE, 'Invalid argument for BEGIN qualifier.')
				}
				opener += ' ' + arguments[0]
	...
	const affirmStmtOpen = function (stmt) {
		if (!stmt.pointer) sqliteError('Stmt has been closed.')
		return stmt
	}
		...
		if (!f._) {
			...
			f._ = {
				string: function (stmt, ndx, val, asBlob) {
					const [pStr, n] = heap.allocCString(val, true)
	...
	Object.defineProperty(DB.prototype, 'pointer', {
		enumerable: true,
		get: function () {
			return __ptrMap.get(this)
		},
		set: () => sqliteError('The pointer property is read-only.'),
	})
