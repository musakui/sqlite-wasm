import { capi, wasm, sqliteError, C_API, structs } from './base.js'
...
export const installSAHPool = (sqlite3) => {
	...
	const ioMethods = {
		...
		xDeviceCharacteristics: function (pFile) {
			return C_API.SQLITE_IOCAP_UNDELETABLE_WHEN_OPEN
		},
	...
	const opfsIoMethods = new structs.sqlite3_io_methods()
	...
	const vfsMethods = {
		...
		xGetLastError: function (pVfs, nOut, pOut) {
			...
			if (e) {
				...
				try {
					const [cMsg, n] = heap.scopedAllocCString(e.message, true)
	...
	class OpfsSAHPool {
		...
		getPath(arg) {
			if (isPtr(arg)) arg = wasm.cstrToJs(arg)
	...
	sqlite3.installOpfsSAHPoolVfs = async function (options = Object.create(null)) {
		...
		return (initPromises[vfsName] = checkOPFS()
			.then(async function () {
				...
				return thePool.isReady
					.then(async () => {
						...
						if (sqlite3.oo1) {
							...
							oo1.DB.dbCtorHelper.setVfsPostOpenSql(theVfs.pointer, function (oo1Db) {
